...
int main ()
{
    ...
    {
        SStory::ContentBody("Es poco después de las 12 de la noche. Te encuentras manejando, en medio de la nada, camino a casa después de una reunión con tus viejos amigos en la ciudad vecina.", 2),
    ...
    myStory.addScene("LLEGADA",{
        SStory::ContentBody("Te detienes a un lado del camino. Al salir notas el silencio que cubre todo el bosque, un escenario perfecto para una caminata. Como siempre, nunca hay red en estas zonas.", 3),
        SStory::ContentBody("Decides marcar donde parqueaste y tomar una corta caminata por el bosque. Después de un rato el camino que seguías se divide en tres. ", 11),
    ...
    myStory.addScene("CAM1",{
        SStory::ContentBody("Te adentras en el camino, la luna está especialmente brillante, cada detalle del suelo resalta.", 12),
    ...
    myStory.addScene("CAM2",{
        SStory::ContentBody("A pesar de lo frondoso del bosque, puedes escuchar la naturaleza que habita el ajeno lugar. Una calma se apodera de tu cuerpo mientras continuas haciendo camino.", 5),
        SStory::ContentBody("La calma se siente cortada al escuchar un disparo a la distancia. ¿Quién mas podria estar en este lugar?", {
    ...
    myStory.addScene("CAM3",{
        SStory::ContentBody("Descendiendo por el camino, llegas al claro de un rio.", 10),
    ...
    myStory.addScene("FINAL",{
        SStory::ContentBody("Cierras los ojos y duermes.", 9),
        ...
        SStory::ContentBody("Inicias el motor y retomas tu camino.",4),
        SStory::ContentBody("Poco después ves un letrero que indica el desvío a casa. Curiosamente notas que el bosque se acaba en el letrero, como si alguien separara los dos lugares con un regla imaginaria. La red del celular regresa y las notificaciones empiezan a llegar.", 13),
        SStory::ContentBody("A pasado una semana.")
    });
    myStory.addScene("INF",{
        SStory::ContentBody("Te levantas y retomas tu camino de regreso al carro.", 11),
        SStory::ContentBody("El camino de regreso se ve distinto del que habías recorrido, pero no le das mucha importancia."),
        SStory::ContentBody("Enciendes el carro y retomas tu camino a casa.", 4),
        SStory::ContentBody("Las horas pasan y sientes que ya has pasado por los mismos lugares y los mismos árboles, hasta que detallas que más adelante están las marcas de la vez que te detuviste a caminar.", 2)
    });
    myStory.addScene("WOLF",{
        SStory::ContentBody("Al acercarte, un nuevo aullido se escucha aun más cerca que antes, pero del lado opuesto, casi como si la criatura supiera que la estas buscando.", 13),
        SStory::ContentBody("Un nuevo ruido, esta vez más agersivo se escucha muy cerca, tal vez ésta no era una muy buena idea después de todo.", 6),
        SStory::ContentBody("Un lobo salta y ataca desde los arbustos, no hay mucho que puedas hacer, un ataque por sorpresa.", 0)
    });
    myStory.addScene("HUNT",{
        SStory::ContentBody("Continuas normal con tu camino."),
        SStory::ContentBody("Otro disparo, pero esta vez te deja sin alientos. Al revisarte, ves como la sangre empieza a fluir del pecho a travez de la ropa.", 7),
